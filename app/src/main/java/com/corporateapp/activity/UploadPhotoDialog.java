    ...
    private void initViews() {
		...
		/*btn_Delete.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				dismiss();
				String network_status = NetworkStatus.checkConnection(context);
				if (network_status.equals("false")) {
					AlertClass alert = new AlertClass();
					alert.customDialog1(context, "No internet connection", "Please check your internet connection");
				}else
				{
					try
					{
						WebServiceImage service = new WebServiceImage(mCallback);
						MultipartEntity reqEntity = new MultipartEntity();

						reqEntity.addPart("user_id", new StringBody(SharedPref.getPreferences().getString(
								SharedPref.USER_ID, "")));

						reqEntity.addPart("delete", new StringBody("Y"));

						reqEntity.addPart("photo", new StringBody(""));

						service.getService(context, Constants.edit_profile, reqEntity);
					} catch (Exception e)
					{
						Log.e("log_tag", "Error data at getDriverListArroundMe funtion in HomeScreen " + e.toString());
					}
				}
			}
		});

		btn_ShowPhoto.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				ShowUserProfilePic objShowUserProfilePic = new ShowUserProfilePic();
				objShowUserProfilePic.customDialogforUserProfilePic(context, SharedPref.getPreferences().getString(
						SharedPref.USER_PROFILEPIC, "") );
			}
		});*/
